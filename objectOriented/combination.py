...
class HatTop(Shape):
    def draw(self):
        self.pid = self.cvns.create_oval(*self.points) #create_oval(*self.points)绘制圆形的方法
...
class Body:
    def __init__(self, cvns, start_point, w, h):
        ...
        self.buttons = []
        self.bo = self.all_button_points()
    ...
    def move_dy(self, points, size):
        y1 = points[1] +size
        y2 = points[3] +size
        return points[0], y1, points[1], y2
